document.addEventListener("DOMContentLoaded", () => {
  // --- ELEMENT SELECTORS ---
  const headerTitle = document.getElementById("header-card-title");
  ...
  const confirmRemoveModal = document.getElementById("modal-confirm-remove");
  const updateToast = document.getElementById("update-toast");
  const removeToast = document.getElementById("remove-toast");

  // Modal Buttons & Form Inputs
            ...
            <div class="card-container">
                ...
                <div class="card-element card-info-right">
                    ...
                    <div class="mastercard-logo">
                        <svg viewBox="0 0 564.51 349.81" xmlns="http://www.w3.org/2000/svg"><path fill="#f79f1a" d="M564.51,174.9c0,96.6 -78.16,174.91 -174.58,174.91S215.36,271.5 215.36,174.9 293.52,0 389.93,0 564.51,78.31 564.51,174.9Z"/><path fill="#ea001b" d="M349.15,174.9c0,96.6 -78.16,174.91 -174.58,174.91S0,271.5 0,174.9 78.16,0 174.57,0 349.15,78.31 349.15,174.9Z"/><path fill="#ff5f01" d="M282.25,37.22a175.18,175.18 0,0 0,0 275.37,175.17 175.17,0 0,0 0,-275.37Z"/></svg>
                    </div>
                </div>
            </div>
        `;
  ...
  const handleUpdateCard = () => {
    ...
    if (isValid) {
      ...
      localStorage.setItem("linkedCardDetails", JSON.stringify(newCardDetails));
      hideModal(linkCardModal);
      showToast(updateToast);
      populatePage(newCardDetails);
    }

    linkCardBtn.disabled = false;
  };

  // --- PAGE INITIALIZATION & DATA POPULATION ---
  const populatePage = (cardDetails) => {
    if (!cardDetails) {
      window.location.href = "wallet.html"; // Redirect if no card data
      return;
    }
    if (headerTitle)
      headerTitle.textContent = `Debit ••${cardDetails.cardNumberLast4}`;
    if (cardPreviewContainer)
      cardPreviewContainer.innerHTML = createPremiumCardHTML(cardDetails);
    if (cardName)
      cardName.innerHTML = `PayPal Debit Card ••${cardDetails.cardNumberLast4}`;
    if (cardExpiryDate) cardExpiryDate.textContent = cardDetails.cardExpiry;
  };

  // --- EVENT LISTENERS ---
  if (editCardBtn) {
    editCardBtn.addEventListener("click", () => {
      const cardDetails = JSON.parse(localStorage.getItem("linkedCardDetails"));
      if (cardDetails && cardExpiryInput) {
        cardExpiryInput.value = cardDetails.cardExpiry;
        cardNumberInput.value = ""; // Clear sensitive fields for re-entry
        cardCvvInput.value = "";
      }
      showModal(linkCardModal);
    });
  }
  ...
  if (confirmRemoveBtn) {
    confirmRemoveBtn.addEventListener("click", () => {
      localStorage.removeItem("linkedCardDetails");
  ...
  if (linkCardBtn) {
    linkCardBtn.addEventListener("click", handleUpdateCard);
  }
  ...
  // --- INITIALIZE THE PAGE ---
  const savedCard = JSON.parse(localStorage.getItem("linkedCardDetails"));
  populatePage(savedCard);
});
